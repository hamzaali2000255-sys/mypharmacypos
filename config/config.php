...
function verify_csrf(?string $token): void { if(!$token||empty($_SESSION['csrf_token'])||!hash_equals($_SESSION['csrf_token'],$token)){http_response_code(419);exit('Invalid or expired request. Please refresh and try again.');} }
...
if(!in_array($publicScript,['login.php','logout.php'],true)){ob_start(function(string $html):string{if(stripos($html,'<header')===false)return $html;$u=current_user()??['username'=>'','role'=>'cashier'];$script=basename($_SERVER['SCRIPT_FILENAME']??'');$dashboardActive=$script==='index.php'?' class="active"':'';$posActive=$script==='pos.php'?' class="active"':'';$managementActive=in_array($script,['manage.php','inventory.php','purchases.php','suppliers.php','customers.php','expenses.php','returns.php','reports.php','cash_drawer.php','backup.php','users.php','audit.php'],true)?' class="active"':'';$header='<header><div class="brand">💊 '.e(APP_NAME).'</div><nav><a'.$dashboardActive.' href="index.php">Dashboard</a><a'.$posActive.' href="pos.php">POS</a><a'.$managementActive.' href="manage.php">Management</a></nav><div class="user-info">👤 '.e($u['username']).' · '.e(ucfirst($u['role'])).' <a href="logout.php">Logout</a></div></header>';return preg_replace('~<header\b[^>]*>.*?</header>~is',$header,$html,1)??$html;});}
